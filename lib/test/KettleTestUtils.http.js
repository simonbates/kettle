 ...
 *  statusCode {Number} The expected status code (defaults to 200 if missing)
 */
kettle.test.assertJSONResponseContains = function (options) {
    var statusCode = options.statusCode || 200;
    var data = kettle.JSON.parse(options.string);
    jqUnit.assertLeftHand(options.message, options.expected, data);
    jqUnit.assertEquals(options.message + " statusCode", statusCode, options.request.nativeResponse.statusCode);
    jqUnit.assertTrue(options.message + " content-type", options.request.nativeResponse.headers["content-type"].indexOf("application/json") === 0);
};
